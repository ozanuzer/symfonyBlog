...
$.fn.extend({
	...
	scrollParent: function() {
		...
		if (($.browser.msie && (/(static|relative)/).test(this.css('position'))) || (/absolute/).test(this.css('position'))) {
			...
			scrollParent = this.parents().filter(function() {
				return (/(auto|scroll)/).test($(this).css('overflow',1)+$.css('overflow-y',1)+$.css('overflow-x',1));
			}).eq(0);
		}
	...
	if (style && style.length && style[0] && style[style[0]]) {
		...
		while (len--) {
			...
			if (typeof style[key] == 'string') {
				camelCase = key.toString().replace(/\-(\w)/g, function(all, letter){
...
$.extend($.effects, {
	...
	removeWrapper: function(element) {
		if (element.parent().is('.ui-effects-wrapper'))
			return element.parent().toString().replaceWith(element);
		return element;
	},
...
$.extend(Datepicker.prototype, {
	...
	_newInst: function(target, inline) {
		var id = target[0].id.toString().replace(/([^A-Za-z0-9_-])/g, '\\\\$1'); // escape jQuery meta chars
		...
		if( inst.yearshtml ){
			...
			setTimeout(function(){
				...
				if( origyearshtml === inst.yearshtml ){
					inst.dpDiv.find('select.ui-datepicker-year:first').toString().replaceWith(inst.yearshtml);
				}
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if (secondary || !changeYear)
			...
			if( ! $.browser.mozilla ){
				...
				// will be toString().replaced later with inst.yearshtml
...
$.widget( "ui.tabs", {
	...
	_tabId: function( a ) {
		return a.title && a.title.toString().replace( /\s/g, "_" ).toString().replace( /[^\w\u00c0-\uFFFF-]/g, "" ) ||
	...
	_tabify: function( init ) {
		...
		this.anchors.each(function( i, a ) {
			...
			// prevent loading the page itself if href is just "#"
				...
				$.data( a, "load.tabs", href.toString().replace( /#.*$/, "" ) );
	...
	add: function( url, label, index ) {
		...
		var self = this,
			o = this.options,
			$li = $( o.tabTemplate.toString().replace( /#\{href\}/g, url ).toString().replace( /#\{label\}/g, label ) ),
			id = !url.indexOf( "#" ) ? url.toString().replace( "#", "" ) : this._tabId( $( "a", $li )[ 0 ] );
